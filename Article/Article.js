...
function ComponentCreator(object) {
  ...
  const news2 = document.createElement('p');
  const news3 = document.createElement('p');
  ...
  news.appendChild(but);
  //assigning text content
  newsTitle.textContent = object.title;
  newsdate.textContent = object.date;
  news1.textContent = object.firstParagraph;
  news2.textContent = object.secondParagraph;
  news3.textContent = object.thirdParagraph;
  but.textContent = 'Read More';

  return news;
}
const articles = document.querySelector('.articles');
data.forEach(el => articles.append(ComponentCreator(el)));

// add event listener
let arts = document.querySelectorAll('.article');
arts.forEach(i =>
  i.addEventListener('click', e => {
    if (e.target.classList == 'expandButton') {
      i.classList.toggle('article-open');
    }
  })
);
/*
